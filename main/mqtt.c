...
void init_mqtt(void)
{
  ...
  if (services->enable_mqtt && strlen(mqtt_address_buff) > 20)
  {
    ...
    if (strlen(services->mqtt_user) == 0) {
      mqtt_cfg.credentials.username = "pi";
    } else {
      mqtt_cfg.credentials.username = services->mqtt_user;
    }

    if (strlen(services->mqtt_password) == 0 ) {
    ...
    timer_publish_status = xTimerCreate("TimerDeviceStatus", 10 * 60 * 1000 / portTICK_PERIOD_MS, pdTRUE, NULL, vTimerCallback);
...
void on_mqtt_message(esp_mqtt_event_handle_t incoming_event) {
  ...
  if (strncmp(command, "set", 3) == 0)
  {
    ...
    ESP_LOGI(TAG,"set duty: %u channel: %lu\n", duty, channel);
  ...
  else if (strncmp(command, "switch", 6) == 0)
  {
    ...
    ESP_LOGI(TAG,"set state channel: %lu to %u\n", channel, state);
